...
echo "2. 设置环境变量：cp backend/.env.example backend/.env"
echo "3. 编辑 .env 文件，设置安全的 JWT_SECRET"
echo "4. 启动服务：docker-compose up -d --build"
echo "5. 查看日志：docker-compose logs -f"
echo ""
echo "重要提醒："
echo "- 必须设置 JWT_SECRET 环境变量，否则服务无法启动"
echo "- JWT_SECRET 必须是安全的随机字符串，不要使用默认值"
echo ""
